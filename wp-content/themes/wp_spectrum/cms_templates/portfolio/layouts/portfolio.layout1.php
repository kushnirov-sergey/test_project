...
?>
<div class="cshero-portfolio-container" style="margin:<?php echo esc_attr($item_margin); ?>;">
    <article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
            ...
            <div class="cshero-portfolio-content-wrap">
                <div class="cshero-portfolio-content-wrap-inner">
                    <div class="cshero-portfolio-meta-box">
                        <?php
                        if ($show_title) {
                            echo '<h4 class="cshero-portfolio-title"><a style="color:'.$title_color.';" title="' . esc_attr(get_the_title()) . '" href="' . esc_url(get_permalink(get_the_ID())) . '" >' . get_the_title() . '</a></h4>';
                        }
        ...
        <div class="link-wrap">
            ...
            if($portfolio_link !='' && $show_link){
                echo '<a class="btn btn-default btn-mini" href="' . esc_url($portfolio_link) . '" target="_blank">'.esc_html($link_text).'</a>';
            }
            if($read_more){
                echo '<a class="btn btn-default btn-mini" title="" href="' . esc_url(get_the_permalink())  . '" >'.esc_html($read_more_text).'</a>';
            }
            if($zoom){
                echo '<a href="'.esc_url($full_image).'" class="btn btn-default btn-mini colorbox">'.esc_html($zoom_text).'</a>';
            }
            ?>
        </div>
    </article>
