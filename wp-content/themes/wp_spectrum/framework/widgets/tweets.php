...
class CS_Tweets_Widget extends WP_Widget {
	...
	function __construct()
	{
		...
		parent::__construct('cs-tweets-widget', __('CS Twitter','wp_spectrum'), $widget_ops, $control_ops);
	}
	...
	function widget($args, $instance)
	{
		...
		$transName = !empty($args['widget_id']) ? 'list_tweets_'.$args['widget_id'] : '' ;
		$cacheTime = 10;
	...
	function form($instance)
	{
		...
		<p>
			...
			<select id="<?php echo ''.$this->get_field_id('show_date'); ?>" class="widefat" name="<?php echo ''.$this->get_field_name('show_date'); ?>">
				<option value="no" <?php if($instance['show_date'] == 'no'){ echo "selected='selected'"; } ?>><?php echo __('No','wp_spectrum'); ?></option>
				<option value="yes" <?php if($instance['show_date'] == 'yes'){ echo "selected='selected'"; } ?>><?php echo __('Yes','wp_spectrum'); ?></option>
			</select>
		</p>
