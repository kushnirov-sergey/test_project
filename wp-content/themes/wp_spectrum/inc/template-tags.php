...
function cshero_paging_nav() {
	...
	$links = paginate_links( array(
			...
			'prev_text' => __( '&larr; Previous', 'wp_spectrum' ),
			'next_text' => __( 'Next &rarr;', 'wp_spectrum' ),
	) );
...
function cshero_post_nav() {
	...
	<nav class="navigation post-navigation clearfix" role="navigation">
		<div class="nav-links">
			<?php
				previous_post_link( '<div class="nav-previous">%link</div>', '<span class="btn btn-primary btn-mini">'.__( 'PREVIOUS POST', 'wp_spectrum' ).'</span>' );
				next_post_link(     '<div class="nav-next">%link</div>',     '<span class="btn btn-primary btn-mini">'.__( 'NEXT POST', 'wp_spectrum' ).'</span>' );
			?>
		</div><!-- .nav-links -->
	</nav><!-- .navigation -->
	<?php
...
function cshero_posted_on() {
	...
	$posted_on = sprintf(
		__( 'Posted on %s', 'post date', 'wp_spectrum' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		__( 'by %s', 'post author', 'wp_spectrum' ),
